...
%\usepackage[a4paper,top=0.5in,left=0.75in,bottom=0.6in,right=0.52in]{geometry}
%\usepackage[a4paper,top=0.5in,left=0.75in,bottom=0.7in,right=0.52in]{geometry}
\usepackage{hyperref}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{bm}
...
%In other words,
%is to greedily use the vertex of the current polyhedron $\mathcal{P}_k$ that minimises the objective~\cite{wulff2013analytic}, 
...
by exploring the dual problem of either the loss-augmented inference or the hinge-loss~\cite{taskar2004dissertation, meshi2010learning, bach2015paired},
...
in the setting of trajectory recommendation, we assume $\Psi(\cdot)$ can be decomposed into singleton and pairwise features, i.e.,
\begin{equation}
...
On the other hand, if we measure the discrepancy using the number of mispredicted POIs (normalised),
\begin{equation}
...
\end{equation}
which is linear as $y_j$ is a constant.
