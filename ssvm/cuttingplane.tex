...
\REPEAT
    \FOR{$i = 1,\dots,n$}
        \STATE $\triangleright$ Query the oracle at point $q = (\mathbf{w}, \bm{\xi})$
        \STATE $\hat{\mathbf{y}} = \argmax_{\bar{\mathbf{y}} \in \mathcal{Y}} \{ \Delta(\mathbf{y}_i, \bar{\mathbf{y}}) + 
               \mathbf{w}^\top \Psi(\mathbf{x}_i, \bar{\mathbf{y}}) \}$ 
               $\triangleright$ Loss-augmented inference
        \STATE $\triangleright$ Check the feasibility of query point $q$ as follows
        \IF{$~\mathbf{w}^\top \Psi(\mathbf{x}_i, \mathbf{y}_i) - \mathbf{w}^\top \Psi(\mathbf{x}_i, \hat{\mathbf{y}}) + \varepsilon < 
            ...
            \STATE $\mathcal{W} = \mathcal{W} \cup \left\{ \mathbf{w}^\top \Psi(\mathbf{x}_i, \mathbf{y}_i) - \mathbf{w}^\top 
                    \Psi(\mathbf{x}_i, \hat{\mathbf{y}}) \ge \Delta(\mathbf{y}_i, \hat{\mathbf{y}}) - \xi_i \right\}$ 
                   $\triangleright$ Update constraints: form a \emph{feasibility cut} if $q$ is infeasible
            \STATE Generate the next query point $q' =(\mathbf{w}, \bm{\xi})$ by solving QP~(\ref{eq:nslackform}) w.r.t. all constraints in $\mathcal{W}$
        \ENDIF
    \ENDFOR
...
\REPEAT
    \STATE Generate query point $q = (\mathbf{w}, \xi)$ by solving QP~(\ref{eq:1slackform}) w.r.t. all constraints in $\mathcal{W}$
    \STATE $\triangleright$ Query the oracle at point $q = (\mathbf{w}, \xi)$ as follows
    \STATE $\hat{\mathbf{y}}_i = \argmax_{\bar{\mathbf{y}} \in \mathcal{Y}} \{ \Delta(\mathbf{y}_i, \bar{\mathbf{y}}) + 
           \mathbf{w}^\top \Psi(\mathbf{x}_i, \bar{\mathbf{y}}) \},~ \forall i~$
           $\triangleright$ Loss-augmented inference
    \STATE $\mathcal{W} = \mathcal{W} \cup \left\{ 
            \frac{1}{n} \sum_{i=1}^n \left( 
            \mathbf{w}^\top \Psi(\mathbf{x}_i, \mathbf{y}_i) - \mathbf{w}^\top \Psi(\mathbf{x}_i, \hat{\mathbf{y}}_i) \right) \ge 
            \frac{1}{n} \sum_{i=1}^n \Delta(\mathbf{y}_i, \hat{\mathbf{y}}_i) - \xi \right\}$
           $\triangleright$ Update constraints: form a \emph{feasibility cut}
...
\begin{itemize}
\item To generate a query point $q$, it solves a QP with respect to all constraints/cuts returned by previous queries. 
...
\item The training algorithm will \emph{stop} if the current query point $q$ is feasible, 
      in other words, it does not form an \emph{objective cut} when $q$ is feasible. 
\end{itemize}
