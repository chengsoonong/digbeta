...
\UNTIL{$\frac{1}{n} \sum_{i=1}^n [ \mathbf{w}^\top \Psi(\mathbf{x}_i, \mathbf{y}_i) - \mathbf{w}^\top \Psi(\mathbf{x}_i, \hat{\mathbf{y}}_i) ] + 
        \varepsilon \ge \frac{1}{n} \sum_{i=1}^n \Delta(\mathbf{y}_i, \bar{\mathbf{y}}_i) - \xi$}
\RETURN $(\mathbf{w}, \xi)$
\end{algorithmic}
\end{algorithm}
