...
We focus on path recommendation tasks where a structured SVM is learned %\footnote{Specifically, we use the {\sc SR} method of that paper.}
...
and the {\bf F$_1$ score on pairs}~\cite{cikm16paper}, which computes the F$_1$ on all ordered pairs in the predicted versus ground truth sequence. %%It is 1 iff both sequences agree completely.
