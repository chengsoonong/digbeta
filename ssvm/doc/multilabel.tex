...
induced by the $n$-th playlist (for simplicity, let $\x\pb{n}$ be the feature vector of the first song in the $n$-th playlist),
...
The empirical risk of a predicting function $\f$ on training set $\SCal$ is
$$
...
Suppose we choose the Hamming loss $\LCal_\text{Hamm}(\cdot)$, by replacing the indicator function in (\ref{eq:loss_hamm_pl}) with 
