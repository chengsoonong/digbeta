...
Given $N$ playlists where songs in each playlist are from a music library with $M$ songs $\{s_i\}_{i=1}^M$,
...
$\y\pb{n} \in \{0,1\}^M$ is a binary indicator such that 
$$
y_i\pb{n} = 
\begin{cases}
...
For training example $(\x, \y)$, let $M_+ = \{i |y_i = 1\}$ and $M_- = \{j |y_j = 0\}$, 
we consider three options for $\LCal(\cdot)$ here:
\begin{enumerate}
\item Hamming loss: 
      \begin{equation}
      \label{eq:loss_hamm_pl0}
      \LCal_\text{Hamm}(\f(\x), \y) = \frac{1}{M} \sum_{i=1}^M \; \llb f_i \ne y_i \rrb
      \end{equation}
\item Rank loss: 
      \begin{equation}
      \label{eq:loss_rank_pl0}
      \LCal_\text{Rank}(\f(\x), \y) = \frac{1}{M_+ \cdot M_-} \sum_{i \in M_+} \sum_{j \in M_-} 
                                      \left( \llb f_i < f_j \rrb + \frac{1}{2} \llb f_i = f_j \rrb \right)
      \end{equation}
\item Top-push loss:
      $$\LCal_\infty(\f(\x), \y) = \frac{1}{M_+} \sum_{i \in M_+} \max_{j \in M_-} \llb f_i < f_j \rrb,$$
      let $\ell(\cdot)$ be a convex surrogate of the indicator function, we have
      \begin{equation}
      \label{eq:loss_inf_pl}
      \widetilde{\LCal}_\infty(\f(\x), \y) = \frac{1}{M_+} \sum_{i \in M_+} \ell\left( \max_{j \in M_-} f_j - f_i \right).
      \end{equation}
\end{enumerate}
...
\paragraph{First song as seed + Hamming loss + Independent logistic regression}
...
Further, we choose the Hamming loss $\LCal_\text{Hamm}(\cdot)$, and by replacing the indicator function in (\ref{eq:loss_hamm_pl0}) with 
...
= \frac{1}{M} \sum_{i=1}^M \left[ -\log \sigma(f_i \cdot y_i) \right]
= \frac{1}{M} \sum_{i=1}^M \log \left(1 + \exp(-f_i \cdot y_i) \right).
\end{equation}
...
&= \frac{1}{2} \w^\top \w + \frac{1}{N} \sum_{n=1}^N \frac{1}{M} \sum_{i=1}^M 
   \log \left(1 + \exp(-f_i\pb{n} \cdot y_i\pb{n}) \right) \\
&= \frac{1}{2} \sum_{i=1}^M \w_i^\top \w_i + \frac{1}{MN} \sum_{n=1}^N \sum_{i=1}^M 
