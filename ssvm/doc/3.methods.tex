...
\noindent
\textit{Integer linear program} \vspace{.5em} \\
If we formulate an ILP to do inference and employ the sub-tour elimination constraints from TSP, we have
\begin{alignat}{5}
& \max_{u,v} ~&& \frac{1}{2} \sum_{k=1}^M \mathbf{w}_k^\top \phi_k(\mathbf{x}, p_k) \sum_{j=1}^M u_{jk} + 
...
For brevity, we arrange the POIs such that $p_1 = p_s$.
Firstly, the desired trajectory should start from $p_s$ (Constraint~\ref{eq:cons2}).
...
i.e., the number of POI visits should be exactly $L$ (including $p_s$).
...
\noindent
\textit{The list Viterbi algorithm} \vspace{.5em} \\
...
\begin{algorithmic}[1]
\STATE \textbf{Input}: $p_s$, $L$, $N$, $\mathbf{w}$, $\Psi$
\STATE Initialise score matrices $\alpha, \beta, f_t, f_{t, t+1}$, max-heap $H$, $k=0$.
\STATE $\triangleright$ Do the forward-backward procedure~\cite{rabiner1989tutorial}
\STATE $\alpha_1(p_j) = 0,~ \beta_L(p_i) = 0,~ p_i, p_j \in \mathcal{P}$
\STATE $\alpha_t(p_j) = \max_{p_i \in \mathcal{P}} \{ \alpha_{t-1}(p_i) + 
        \mathbf{w}_{ij}^\top \Psi_{ij}(\mathbf{x}, p_i, p_j) + \mathbf{w}_j^\top \Psi_j(\mathbf{x}, p_j) \},~ p_j \in \mathcal{P},~ t=2,\dots,L$
\STATE $\beta_t(p_i) = \max_{p_j \in \mathcal{P}} \{ \mathbf{w}_{ij}^\top \Psi_{ij}(\mathbf{x}, p_i, p_j) + 
        \mathbf{w}_j^\top \Psi_j(\mathbf{x}, p_j) + \beta_{t+1}(p_j) \},~ p_i \in \mathcal{P},~ t = L-1,\dots,1$
\STATE $f_t(p_i) = \alpha_t(p_i) + \beta_t(p_i),~ p_i \in \mathcal{P},~ t = 1,\dots,L$
\STATE $f_{t,t+1}(p_i, p_j) = \alpha_t(p_i) + \mathbf{w}_{ij}^\top \Psi_{ij}(\mathbf{x}, p_i, p_j) + 
                              \mathbf{w}_j^\top \Psi_j(\mathbf{x}, p_j) + \beta_{t+1}(p_j),~ p_i, p_j \in \mathcal{P},~ t = 1,\dots,L-1$
\STATE $\triangleright$ Identify the best (scored) trajectory $\mathbf{y}^1$ (possibly with sub-tours)
\STATE $y_1^1 = p_s,~ y_2^1 = \argmax_{p \in \mathcal{P}} f_{1,2}(p_s, p)$
\STATE $y_t^1 = \argmax_{p \in \mathcal{P}} f_{t-1,t}(y_{t-1}^1, p),~ t = 3,\dots,L$
\STATE \textit{priority} = $\max_{p \in \mathcal{P}} f_L(p)$, \textit{partition\_index} = \textsc{nil}, \textit{exclude\_set} = $\emptyset$
\STATE $H$.\textit{push}(\textit{priority}, \textit{partition\_index}, \textit{exclude\_set}, $\mathbf{y}^1$)
\WHILE{$H$ is not empty \textbf{and} $k < N^{L-1} - \prod_{t=2}^L (N-t+1)$}
    \STATE \textit{priority}, \textit{partition\_index}, \textit{exclude\_set}, $\mathbf{y}^k$ = $H$.pop(); $k = k + 1$
    \IF{No sub-tours in $\mathbf{y}^k$}
        \RETURN $\mathbf{y}^k$
    \ENDIF
    \STATE $\triangleright$ Identify the $(k+1)$-th best (scored) trajectory $\mathbf{y}^{k+1}$ (possibly with sub-tours)
    \IF{\textit{partition\_index} $\ne$ \textsc{nil}}
        \STATE \textit{partition\_index\_start} = \textit{partition\_index}
    \ELSE
        \STATE \textit{partition\_index\_start} = $2$
    \ENDIF
    \FOR{$t = \textit{partition\_index\_start},\dots,L$}
        \STATE \textit{new\_exclude\_set} = $\{ y_t^k \}$
        \IF{$t = \textit{partition\_index\_start}$}
            \STATE \textit{new\_exclude\_set} = \textit{new\_exclude\_set} $\cap$ \textit{exclude\_set}
        \ENDIF
        \STATE $\bar{y}_j = \begin{cases}
                            y_j^k,~~ j=1,\dots,t-1 \\
                            \argmax_{p \in \mathcal{P} \setminus \textit{new\_exclude\_set}} f_{t-1,t}(y_{t-1}^k, p),~ j=t \\
                            \argmax_{p \in \mathcal{P}} f_{j-1, j}(\bar{y}_{j-1}, p),~ j=t+1,\dots,L
                \end{cases}$
        \STATE \textit{new\_priority} = $f_{t-1,t}(y_{t-1}^k, \bar{y}_t)$
        \IF{\textit{partition\_index} $\ne$ \textsc{nil}}
            \STATE \textit{new\_priority} += \textit{priority} - $f_{t-1,t}(y_{t-1}^k, y_t^k)$
        \ENDIF

        $H$.\textit{push}(\textit{new\_priority}, $t$, \textit{new\_exclude\_set}, $\bar{\mathbf{y}}$)
    \ENDFOR
\ENDWHILE
\end{algorithmic}
\end{algorithm}
