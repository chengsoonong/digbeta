...
$}
\end{equation}
%where $\bar{\y}^{(i)} \in \mathcal{Y} \setminus \{\y^{ij}\}_{j=1}^{n_i}$. % DW: no need to repeat this
This one-slack formulation can be trained more efficiently than the $N$-slack formulation (\ref{eq:nslack_ml}) and is employed in this work.

%\TODO{explain how 1-slack connects to n-slack and this is what's implemented} % DW: kind of done
