...
+ \sum_{j,k=1}^L u_{jk} \w_{jk}^\top \Psi_{j, k}(\x, y_j, y_k) \nonumber\\
  %              & + \sum_{j=1}^l \sum_{k=1}^l u_{jk} \w_{jk}^\top \Psi_{j, k}(\x, y_j, y_k) \\
s.t. ~& u_{jk}, ~z_j \in \{0, 1\}, ~u_{jj}=0, ~z_1=0, ~v_j \in \mathbf{Z},~ \\
  & y_j \in \{1,\dots,L\}, ~\forall j, k = 1,\cdots,L   \label{eq:cons1} \\
  & \sum_{k=2}^L u_{1k} = 1, ~\sum_{j=2}^L u_{j1} = 0  \label{eq:cons2} \\
  & \sum_{j=1}^L u_{ji} = z_i + \sum_{k=2}^L u_{ik} \le 1,  ~\forall i=2,\cdots,L                    \label{eq:cons3} \\
  & \sum_{j=1}^M \sum_{k=1}^L u_{jk} = l-1,                                                           \label{eq:cons4} \\
  & v_j - v_k + 1 \le (L-1) (1-u_{jk}),                     \forall j,k=2,\cdots,L                    \nonumber
\end{alignat}
\end{minipage}
}
...
The algorithm that seem closest to loss-augmented inference with exclusions are several variants 
