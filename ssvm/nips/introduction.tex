...
\rev{incorporating multiple correct sequences into a structure prediction formulation,} 
...
\rev{and by two novel applications of the ListViterbi algorithm that sequentially finds the list of top-scoring sequences.}
...
\begin{itemize}[noitemsep,leftmargin=12pt]\itemmoveup
    ...
    \item We propose \rev{two} novel application of the list Viterbi algorithm, an extension of the classic Viterbi algorithm that returns a list of the highest scored sequences under some model, \rev{to predict sequences without repeated elements (i.e. {\em path}s in state-space), and to} exclude multiple ground truths for model learning (\S\ref{ssec:training}).
