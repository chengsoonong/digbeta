...
followed by algorithms for its learning (Sec~\ref{ssec:subtour}) and inference (Sec~\ref{ssec:SRinf}).
...
If training using cutting-plane algorithm, the above can be learned more efficiently than (\ref{eq:nslack_ml}).
...
%\resizebox{.99\columnwidth}{!}{$\displaystyle
%%  \begin{minipage}{\linewidth}
\begin{alignat}{5}
...
and also allows excluding an arbitrary number of known sequences. %by additionally checking, when we get each of the $k$th best sequence, whether or not it is in the exclusion set.
