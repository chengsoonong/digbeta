...
%we are 
%provided as input
\rev{learned from} a training set
...
where for each trajectory query $\x^{(i)}$ there is a list of \rev{ranked} POI pairs
...
This method is feasible because one is only interested in predicting sequences one element at a time, \rev{instead of predicting the entire sequence given the initial item.}
%so that latent embeddings for each item may be found.
...
We now see how to do so \rev{with novel extensions to} structured prediction approaches. %by attacking the problem using tools from structured prediction.
