...
In particular,
$\mathbf{y} = (s,~ y_2, \dots, y_l)$ is a trajectory with $l$ distinct POIs. %, which has no sub-tours. %i.e. $y_j \ne y_k$ if $j \ne k$.
...
%
\subsection{The case for exploiting structure in sequence recommendation}
...
in particular,
they are not capable of ensuring the \emph{global} cohesion of the corresponding recommended outputs $\y$.
%as they inherently rely on either pointwise or pairwise preferences for POIs.
For example, consider the RankSVM baseline in the trajectory recommendation problem. %as argued in Section~\ref{sec:intro},
Here, we might find three restaurants to be the highest scoring POIs in a city;
