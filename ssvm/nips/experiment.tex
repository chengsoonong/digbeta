...
Music playlist datasets are plausible, but recent results show that sequencing information does not affect the data likelihood~\cite{chen2012playlist}. 
}
%LX - this is verbose
...
a single POI are excluded. \rev{We use 12 behavioural and geographical features to describe each POI and pairs of POIs (see supplement).}
\rev{Table~\ref{tab:data} summarises the profile of each dataset. 
...
\rev{We consider four variants of sequence recommendation, starting with a structured prediction model, then incorporating multiple ground truths, and finally path constraints:}
\begin{itemize}[leftmargin=0.125in]\itemmoveup
\parskip -.05em
...
\rev{Table~\ref{tab:result} and Figure~\ref{fig:topk} summarise results for structured recommendation approaches and baselines. See the supplement for results across all datasets on all metric variants. Reassuringly, we see that the best and next best approaches uses structured prediction and their path variants. In particular,}
...
Overall, \rev{structured recommendation methods have shown superior performance in location sequence recommendation tasks on a public benchmark. Most notably, taking into account multiple ground truths in structured prediction and modeling path constraints are important; and the effects are more pronounced in longer trajectories. The sparsity of this dataset presents a barrier for large-scale evaluations, and this is left for further exploration.}
Finally, we note that the unary terms in the sequence scoring function \eqref{eq:jointfeature} can be replaced with {\em personalised} terms to each user, such as from a recommender system~\cite{Koren:2009,bpr09}; we leave this, and joint personalisation and structured recommendation, as future work.
