...
of its POIs as well as its likelihood is optimising the following objective:
\begin{equation*}
...
We call this approach %that uses both the ranking of POIs and POI-POI transitions 
\textsc{Rank+Markov},
...
& s.t. ~&& x_{ij} \in \{0, 1\}, ~u_i \in \mathbf{Z},            ~\forall i, j = 1, \cdots, M \label{eq:cons1} \\
&       && \sum_{j=2}^M x_{1j} = \sum_{i=1}^{M-1} x_{iM} = 1                                 \label{eq:cons2} \\
