...
\end{aligned}
\end{equation}
and objective~(\ref{eq:tpush_obj}) can be approximated as
\begin{equation}
\label{eq:tpush_obj_approx}
J(\w) \approx \frac{\lambda}{2} \w^\top \w + \frac{1}{N} \sum_{n=1}^N \sum_{k=1}^K \frac{\llb y_k^n = 1 \rrb}{K_+^n} 
              \log \left[ 1 + \exp(-\w_k^\top \x^n) \left[ \underset{j:y_j^n=0}{\sum} \exp(r \w_j^\top \x^n) \right]^{1/r} \right]
\end{equation}
...
\approx \frac{\llb y_k = 1 \rrb} {K_+} \cdot
        \frac{-\x \cdot \left[ \underset{j:y_j=0}{\sum} \exp(r \w_j^\top \x) \right]^{1/r}}
             {\exp(\w_k^\top \x) + \left[ \underset{j:y_j=0}{\sum} \exp(r \w_j^\top \x) \right]^{1/r}}
$$
...
&\approx \lambda \w_k + \frac{1}{N} \sum_{n=1}^N 
         ...
         \frac{-\x^n \cdot \left[ \underset{j:y_j^n=0}{\sum} \exp(r \w_j^\top \x^n) \right]^{1/r}}
              {\exp(\w_k^\top \x^n) + \left[ \underset{j:y_j^n=0}{\sum} \exp(r \w_j^\top \x^n) \right]^{1/r}}
\end{align*}
