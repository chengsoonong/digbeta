...
$$
\B = \left[ \left( \one_{N \times K} - \Y \right) \circ \exp \left( r \X \W^\top \right) \right] \one_K
$$
...
To vectorise the gradients, we first rewrite (\ref{eq:grad_of_loss_pos}) and (\ref{eq:grad_of_loss_neg}) as
$$
...
& \text{if} \ y_k = 1 \\ \\
%
\displaystyle\frac{\x \, \exp \left( r \w_k^\top \x \right)}{K_+ \underset{j:y_j=0}{\sum} \exp \left(r \w_j^\top \x \right) } 
\sum_{i:y_i=1}
\left[ 1 + \exp \left( \w_i^\top \x \right) \left[ \underset{j:y_j=0}{\sum} \exp \left(r \w_j^\top \x \right) \right]^{-\frac{1}{r}} \right]^{-1}, 
& \text{if} \ y_k = 0
\end{cases}
$$
...
\begin{align*}
\G_+ 
&= \left[ (\A \Y) \circ \left[ \one_{N \times K} + \widetilde\B^{\circ (1/r)} \exp(\X \W^\top) \right]^{\circ (-1)} \right]^\top (-\X) \\
\G_- 
&= \left[ \A \widetilde\B \Omegabm \left[ (\one_{N \times K} - \Y) \circ \exp(r \X \W^\top) \right] \right]^\top \X
\end{align*}
where $\Omegabm \in \R^{N \times N}$ is a diagonal matrix such that
$$
\Omegabm_{n,n}
= \sum_{i:y_i^n=1} \left[ 1 + 
  \exp \left( \w_i^\top \x^n \right) \left[ \underset{j:y_j^n=0}{\sum} \exp \left(r \w_j^\top \x^n \right) \right]^{-\frac{1}{r}} \right]^{-1}
$$
and $\Omegabm$ can be computed as
$$
\Omegabm = \left[ \Y \circ \left[ \one_{N \times K} + \widetilde\B^{\circ (1/r)} \exp(\X \W^\top) \right]^{\circ (-1)} \right] \one_{K}
$$
