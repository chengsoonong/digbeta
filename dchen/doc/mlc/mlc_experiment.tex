...
%song features: genre, age/year of release, artist, lyrics, audio features, album, user rating etc.
%user features: age, gender, occupation, preference (genre, artist, etc), music purchase history etc.

\eat{
In summary, the 

 - specify what a query is, and the input/output of the predictor
input: query features
output: a indicator vector, a distribution, a ranking

determine the number of songs to recommend:
specify in the query
fit a distribution of the number of songs in training set, sampling a number from this distribution
for ranking based method, require that there should be a margin between ranking scores of recommended songs and those not recommended.

Alternative formulations:
instead of ranking, 
sequential prediction problem (sequentially predict each label given previously predicted labels)
}
