...
\item \textsc{PoiRank}~\cite{cikm16paper}
      ...
      and trains a RankSVM model%~\eqref{eq:ranksvm} 
...
Figure~\ref{fig:example} shows an example of a recommended trajectory by the SR model, 
...
\begin{figure}[!h]
    ...
    \end{subfigure}
    \caption{Example of a recommended trajectory (left); A radar chart compares scores of two POIs decomposed according to features (right).}
    \label{fig:example}
\end{figure}
