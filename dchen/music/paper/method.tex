...
\end{equation}
%where the regularisation term $\Omega(\uptheta)$ is defined in (\ref{eq:reg}).
...
%\begin{equation}
%\label{eq:obj}
%\min_\uptheta \ \Omega(\uptheta) + \frac{1}{N} \sum_{i=1}^U \sum_{j \in P_i} \frac{1}{M_-^j} \sum_{m': y_{m'}^j = 0} 
