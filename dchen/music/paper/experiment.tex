...
We split the two playlist datasets into training and test set, %for each of the three cold-start settings,
...
and a user will thus not be used for test %included in the test set 
...
which suggests that MTC might degenerate to simply rank songs according to popularity when making recommendation
...
and cold users setting (when simple attributes of new users are available),
...
which indicates that MTC trends to recommend popular songs to form new playlists. %for existing users.
To conclude, 
