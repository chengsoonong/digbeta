...
class QPMTR(object):
    ...
    def fit(self, w0=None, verbose=0):
        ...
        if verbose > 0:
            t1 = time.time()
            print('%d by %d sparse matrix created in %.1f seconds.' % (P.shape[0], P.shape[1], t1 - t0))
        ...
        if verbose > 0:
            t2 = time.time()
            print('%d dense vector created in %.1f seconds.' % (q.shape[0], t2 - t1))
        ...
        if verbose > 0:
            print('Solving QP: %d variables, %d constraints ...\n' % (self.num_vars, self.num_cons))
