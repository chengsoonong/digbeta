...
class MTR(object):
    def __init__(self, X_train, Y_train, C1, C2, C3, cliques, verbose=0):
        ...
        self.G0, self.h0 = self._create_Gh()
    ...
    def _create_Gh(self):
        ...
        # i.e. -\delta_k <= 0
        G0.append(np.full(N, -1., dtype=np.float))
        rows.append(np.arange(ix, ix + N))
        cols.append(np.arange(self.num_vars - N, self.num_vars))

        G0 = cvxopt.spmatrix(np.concatenate(G0, axis=-1),
                             np.concatenate(rows, axis=-1),
                             np.concatenate(cols, axis=-1),
                             size=(2 * N, self.num_vars))
        # h0 = cvxopt.spmatrix(-Mplus, np.arange(N), np.zeros(N, dtype=np.int32),
        #                      size=(2 * N, 1))
        h0 = cvxopt.matrix(0., size=(2 * N, 1))  # should be dense matrix
        h0[:N] = -Mplus
        return G0, h0

    def _update_Gh(self):
        if self.get_num_constraints() == 0:
            return self.G0, self.h0

        N, D, U = self.N, self.D, self.U
        rows, cols, values = [self.G0.I], [self.G0.J], [self.G0.V]
        ix = self.G0.size[0]
        for k in range(N):
            u = self.pl2u[k]
            for n in sorted(self.current_constraints[k]):
                ...
                vec = self.X[n, :]
                values += cvxopt.matrix(np.r_[vec, vec, vec, [-1.]])
                rows.append(cvxopt.matrix(np.full(3 * D + 1, ix, dtype=np.int32)))
                cols.append(cvxopt.matrix(np.arange(D)))
                cols.append(cvxopt.matrix(np.arange((u + 1) * D, (u + 2) * D)))
                cols.append(cvxopt.matrix(np.arange((U + 1 + k) * D, (U + 2 + k) * D)))
                cols.append(cvxopt.matrix([(U + N + 1) * D + k]))
                ix += 1
        G = cvxopt.spmatrix(cvxopt.matrix(values),
                            cvxopt.matrix(rows),
                            cvxopt.matrix(cols),
                            size=(ix, self.num_vars))
        # h = cvxopt.spmatrix(self.h0.V, self.h0.I, self.h0.J, size=(ix, 1))
        h = cvxopt.matrix(0., size=(ix, 1))
        h[:N] = self.h0[:N]
        return G, h
    ...
    def fit(self, max_iter=1e3, use_all_constraints=False, w0=None, fnpy=None):
        ...
        while cp_iter < max_iter:
            ...
            try:
                solution = cvxopt.solvers.qp(self.P, self.q, G, h, initvals=self._init_vars())
            except ValueError:
