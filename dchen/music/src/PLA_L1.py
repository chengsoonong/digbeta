...
if trndev == 'N':
    fxtrain = os.path.join(data_dir, 'X_train_%d.pkl.gz' % n_seed)
    ...
    fprefix = 'plal1-%g-%g-%g' % (n_seed, C, p)
else:
    fxtrain = os.path.join(data_dir, 'X_trndev_%d.pkl.gz' % n_seed)
    ...
    fprefix = 'trndev-plal1-%g-%g-%g' % (n_seed, C, p)
...
if os.path.exists(fmodel):
    ...
    clf = MTC_L1(X_train, Y, C=C, p=p, user_playlist_indices=cliques)
